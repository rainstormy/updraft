...
* *Unit tests* via https://vitest.dev[Vitest] (`validate.test`).
** `validate.test.coverage` creates a code coverage report in the `node_modules/.vitest/coverage` directory.
** `validate.test.overview` launches the https://vitest.dev/guide/ui.html[Vitest user interface] in a web browser.
...
It saves the output in the `dist` directory.
